...
此脚本用于运行 Supabase 相关的测试，包括连接测试、API 测试和数据迁移测试。
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"logs/supabase_tests_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='运行 Supabase 测试')
