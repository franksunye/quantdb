    ...
    def check_lightweight_design(self) -> bool:
        ...
        for py_file in self.qdb_path.glob("*.py"):
            ...
            if "create_all" in content or "Base.metadata" in content:
                violations.append(
                    f"{py_file.name}: Contains database initialization logic"
                )

            # Check for service instantiation
            if "Service(" in content and "=" in content:
                violations.append(
                    f"{py_file.name}: Contains service instantiation logic"
                )
            ...
            for i, line in enumerate(lines):
                if "if" in line and ("days" in line or "start_date" in line):
                    ...
                    if i + 1 < len(lines) and (
                        "timedelta" in lines[i + 1] or "strftime" in lines[i + 1]
                    ):
                        violations.append(
                            f"{py_file.name}:{i+1}: Contains parameter processing logic"
                        )
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        ...
                        if self._has_complex_logic(node):
                            violations.append(
                                f"{py_file.name}:{node.lineno}: Function '{node.name}' contains business logic"
                            )
    ...
    def check_service_initialization(self) -> bool:
        ...
        for py_file in self.qdb_path.glob("*.py"):
            ...
            if "from core.services" in content and "Service(" in content:
                ...
                if (
                    "ServiceManager" not in content
                    and "get_service_manager" not in content
                ):
                    violations.append(
                        f"{py_file.name}: Direct service instantiation without ServiceManager"
                    )
    ...
    def check_code_reuse(self) -> bool:
        ...
        if len(init_patterns) > 1:
            ...
            if not_using_service_manager:
                duplications.append(
                    f"Service initialization patterns duplicated in: {', '.join(not_using_service_manager)} (should use ServiceManager)"
                )
    ...
    def check_dependency_direction(self) -> bool:
        ...
        for py_file in self.core_path.glob("**/*.py"):
            ...
            if "from qdb" in content or "import qdb" in content:
                violations.append(
                    f"{py_file.relative_to(self.project_root)}: Core imports from qdb (wrong direction)"
                )
        ...
        for node in ast.walk(func_node):
            if isinstance(node, (ast.If, ast.For, ast.While, ast.Try)):
                ...
                if hasattr(node.func, "attr") and node.func.attr in [
                    ...
                    complex_nodes += 1
                elif hasattr(node.func, "id") and node.func.id in [
                    "timedelta",
                    "datetime",
                ]:
                    complex_nodes += 1
