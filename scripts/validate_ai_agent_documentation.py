    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        for func_name, result in results.items():
            ...
            if result.get("has_docstring"):
                ...
                report.append(
                    f"   📋 Args section: {'✅' if result.get('has_args_section') else '❌'}"
                )
                report.append(
                    f"   📤 Returns section: {'✅' if result.get('has_returns_section') else '❌'}"
                )
                report.append(
                    f"   ⚠️  Raises section: {'✅' if result.get('has_raises_section') else '❌'}"
                )
                report.append(
                ...
                if param_count > 0:
                    ...
                    report.append(
                        f"   📊 Parameter coverage: {documented_params}/{param_count} ({coverage:.0f}%)"
                    )
            else:
