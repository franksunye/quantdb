...
## [2.2.8] - 用户体验优化+API兼容性增强 (2025-08-07)

### 🔧 用户体验优化
- **API接口增强**: `get_stock_data()` 现在支持位置参数调用
  - 新增支持: `qdb.get_stock_data('000001', '20240101', '20240131')`
  - 保持兼容: `qdb.get_stock_data('000001', start_date='20240101', end_date='20240131')`
  - 混合调用: `qdb.get_stock_data('000001', '20240101', end_date='20240131')`
- **用户体验测试**: 创建完整的用户接受度测试框架
- **文档更新**: README_PYPI.md 更新最新功能和API示例，新增“Feature details”小节，Advanced Features 增加“指数”示例
- **API导出**: 在 `qdb.__init__` 顶层导出 `get_index_data`/`get_index_realtime`/`get_index_list`

### 🧪 测试质量提升
- **100%测试通过**: 149个测试全部通过，0个失败
- **用户场景验证**: 从新手到专业用户的完整使用流程测试
- **API兼容性验证**: 多种调用模式的兼容性测试

### 📦 发布准备
- **版本统一**: 所有文件版本号统一更新至v2.2.8
- **PyPI就绪**: 准备发布包含用户体验优化的稳定版本
