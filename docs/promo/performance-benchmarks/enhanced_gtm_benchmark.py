...
def run_comprehensive_benchmark():
    ...
    for result in benchmark_results:
        result['improvement_vs_akshare'] = ((result['akshare_direct'] - result['quantdb_cache']) / result['akshare_direct']) * 100
        result['improvement_vs_first'] = ((result['quantdb_first'] - result['quantdb_cache']) / result['quantdb_first']) * 100
...
def create_enhanced_performance_chart(benchmark_results):
    ...
    bars2 = ax1.bar(x, quantdb_first_times, width, label='QuantDB First Call', 
                    color='#F39C12', alpha=0.8, edgecolor='white', linewidth=1)
    bars3 = ax1.bar(x + width, quantdb_cache_times, width, label='QuantDB Cache Hit', 
    ...
    bars = ax2.bar(scenario_short, improvements_akshare, color=colors, alpha=0.8,
    ...
    bars = ax3.barh(scenario_short, speedup_factors, color=['#2ECC71', '#3498DB', '#9B59B6'], 
    ...
    for bar, speedup in zip(bars, speedup_factors):
        width = bar.get_width()
        ax3.annotate(f'{speedup:.0f}× faster',
    ...
    scatter = ax4.scatter(records, cache_times_ms, s=200, c=colors, alpha=0.8, edgecolors='white', linewidth=2)
    ...
    for i, (record, cache_time, scenario) in enumerate(zip(records, cache_times_ms, scenario_short)):
        ax4.annotate(f'{scenario}\n{cache_time:.1f}ms',
...
def create_executive_summary_chart(benchmark_results):
    ...
    # Metric boxes
    metrics = [
        {'title': 'Average Performance\nImprovement', 'value': f'{avg_improvement:.1f}%', 'color': '#27AE60'},
        {'title': 'Maximum Speedup\nFactor', 'value': f'{max_speedup:.0f}×', 'color': '#3498DB'},
        {'title': 'Average Cache\nResponse Time', 'value': f'{avg_cache_time*1000:.1f}ms', 'color': '#E74C3C'},
    ]
    ...
    for i, metric in enumerate(metrics):
        ...
        # Value
        ax.text(x_pos + box_width/2, y_pos + box_height*0.6, metric['value'], 
               ha='center', va='center', fontsize=32, fontweight='bold', 
               color=metric['color'])
        
        # Title
        ax.text(x_pos + box_width/2, y_pos + box_height*0.2, metric['title'], 
               ha='center', va='center', fontsize=14, fontweight='bold')
    
    # Detailed results
    table_y = 0.5
    ax.text(0.5, table_y, 'Detailed Benchmark Results', ha='center', va='top', 
    ...
    for i, header in enumerate(headers):
        ax.text(0.1 + i * 0.16, table_y, header, ha='center', va='center', 
               fontsize=12, fontweight='bold')
    
    for row_idx, row in enumerate(table_data):
        row_y = table_y - 0.06 * (row_idx + 1)
        for col_idx, cell in enumerate(row):
            ax.text(0.1 + col_idx * 0.16, row_y, cell, ha='center', va='center', 
                   fontsize=11)
