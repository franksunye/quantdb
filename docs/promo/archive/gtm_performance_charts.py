...
def create_performance_comparison():
    ...
    scenarios = ['Single Stock\n(30 days)', 'Multiple Stocks\n(3 stocks, 30 days)', 'Large Dataset\n(5 stocks, 90 days)']
    ...
    bars1 = ax1.bar(x - width, akshare_times, width, label='AKShare Direct', 
                    color='#FF6B6B', alpha=0.8, edgecolor='white', linewidth=1)
    bars2 = ax1.bar(x, quantdb_first, width, label='QuantDB First Call', 
                    color='#4ECDC4', alpha=0.8, edgecolor='white', linewidth=1)
    bars3 = ax1.bar(x + width, quantdb_cache, width, label='QuantDB Cache Hit', 
    ...
    improvements_vs_akshare = [(akshare - cache) / akshare * 100 
                              for akshare, cache in zip(akshare_times, quantdb_cache)]
    
    colors = ['#2ECC71', '#3498DB', '#9B59B6']
    bars = ax2.bar(scenarios, improvements_vs_akshare, color=colors, alpha=0.8, 
    ...
    plt.tight_layout()
    plt.savefig('quantdb_vs_akshare_performance.png', dpi=300, bbox_inches='tight', 
...
def create_speedup_chart():
    ...
    # Speedup data
    scenarios = ['Single Stock (30 days)', 'Multiple Stocks (3×30 days)', 'Large Dataset (5×90 days)']
    ...
    bars = ax.barh(scenarios, speedup_factors, color=colors, alpha=0.8, 
    ...
    plt.tight_layout()
    plt.savefig('quantdb_speedup_factors.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
...
def create_gtm_infographic():
    ...
    # Title
    fig.suptitle('QuantDB: Intelligent Stock Data Caching for Python', 
                fontsize=28, fontweight='bold', y=0.95)
    
    # Subtitle
    fig.text(0.5, 0.91, 'Performance Benchmark Results & Key Benefits', 
    ...
    # Performance metrics
    metrics = [
        {'title': 'Average Performance\nImprovement', 'value': '98.7%', 'color': '#2ECC71', 'icon': '⚡'},
    ...
    for i, metric in enumerate(metrics):
        ...
        # Icon
        ax1.text(x_pos, y_pos + 0.15, metric['icon'], ha='center', va='center', 
                fontsize=40)
        
        # Value
        ax1.text(x_pos, y_pos, metric['value'], ha='center', va='center', 
                fontsize=24, fontweight='bold', color=metric['color'])
        
        # Title
        ax1.text(x_pos, y_pos - 0.15, metric['title'], ha='center', va='center', 
    ...
    bars1 = ax2.bar(x - width/2, akshare_times, width, label='AKShare Direct', 
                    color='#FF6B6B', alpha=0.8)
    bars2 = ax2.bar(x + width/2, quantdb_cache, width, label='QuantDB Cache', 
    ...
    ax3.text(0.5, 0.9, 'Key Features', ha='center', va='top', 
             fontsize=16, fontweight='bold', transform=ax3.transAxes)
    
    for i, feature in enumerate(features):
        y_pos = 0.8 - i * 0.1
        ax3.text(0.1, y_pos, feature, ha='left', va='center', 
...
stats = qdb.cache_stats()  # Monitor cache performance'''
    ...
    ax4.text(0.5, 0.5, code_example, ha='center', va='center', 
    ...
    plt.savefig('quantdb_gtm_infographic.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
...
def create_roi_chart():
    ...
    ax1.plot(daily_requests, [t/60 for t in akshare_time], 'o-', label='AKShare Direct', 
             linewidth=3, markersize=8, color='#E74C3C')
    ax1.plot(daily_requests, [t/60 for t in quantdb_time], 'o-', label='QuantDB Cache', 
    ...
    # Time savings
    ax2.bar(daily_requests, [t/3600 for t in time_saved], color='#3498DB', alpha=0.8,
            edgecolor='white', linewidth=1)
    ...
    for i, (req, saved) in enumerate(zip(daily_requests, time_saved)):
        if saved/3600 > 0.1:  # Only show label if > 6 minutes
            ax2.annotate(f'{saved/3600:.1f}h',
                        xy=(req, saved/3600),
                        xytext=(0, 3),
                        textcoords="offset points",
                        ha='center', va='bottom', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('quantdb_roi_analysis.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
