    ...
    for symbol, name in hk_indexes.items():
        ...
        try:
            ...
            if not df.empty:
                ...
                print(
                    f"   📈 Period change: {((df.iloc[-1]['close'] / df.iloc[0]['close']) - 1) * 100:+.2f}%"
                )
    ...
    for symbol in hk_indexes:
        try:
            ...
            if quote:
                ...
                print(
                    f"{change_color} {symbol:8} | {quote['name']:12} | {quote['price']:>10,.2f} | {quote['change']:>+8.2f} ({quote['pct_change']:>+6.2f}%)"
                )
            else:
