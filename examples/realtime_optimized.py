...
class RealtimeDataManager:
    ...
    def get_batch_optimized(self, symbols: List[str], force_refresh: bool = False) -> Dict[str, Dict[str, Any]]:
        """优化的批量获取 - 使用智能缓存和并发"""
        now = time.time()
        result = {}
        symbols_to_fetch = []
        ...
        if symbols_to_fetch:
            try:
                # 使用原生批量接口
                batch_data = qdb.get_realtime_data_batch(symbols_to_fetch, force_refresh=force_refresh)
...
def benchmark_comparison():
    ...
    for symbol in test_symbols:
        ...
        print(f"  {symbol}: {data.get('current_price', 'N/A')} (缓存: {'是' if data.get('cache_hit') else '否'})")
            ...
            for symbol in watch_list:
                if symbol in data:
                    ...
                    if 'error' not in info:
                        ...
                        print(f"  {cache_icon} {symbol}: ¥{price:.2f} {trend_icon}{change:+.2f}({change_pct:+.2f}%)")
                    else:
