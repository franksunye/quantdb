...
class MockQDB:
    ...
    def get_realtime_data(self, symbol, force_refresh=False):
        ...
        # 模拟股票数据
        mock_data = {
            "000001": {"name": "平安银行", "current_price": 10.50, "change": 0.15, "change_percent": 1.45},
            "000002": {"name": "万科A", "current_price": 25.30, "change": -0.20, "change_percent": -0.78},
            "600000": {"name": "浦发银行", "current_price": 8.20, "change": 0.05, "change_percent": 0.61},
            "000858": {"name": "五粮液", "current_price": 158.50, "change": 2.30, "change_percent": 1.47},
            "002415": {"name": "海康威视", "current_price": 32.80, "change": -0.50, "change_percent": -1.50},
        }
...
def performance_comparison():
    ...
    if time2 > 0:
        ...
        print(f"  缓存 vs 网络获取: {time3/time4:.2f}x 速度提升" if time4 > 0 else "  缓存: 无网络调用!")
