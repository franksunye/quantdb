...
def get_stock_recommendations(exclude_symbol: str = None, market: str = "all") -> List[Dict[str, str]]:
    """
    获取推荐的股票列表 - 支持A股和港股
...
def get_alternative_suggestions(symbol: str) -> List[Dict[str, str]]:
    ...
    if symbol.isdigit() and len(symbol) == 5:
        ...
        suggestions.extend([
            ...
            {"symbol": "09988", "name": "阿里巴巴-SW", "reason": "港股互联网巨头", "market": "港股"},
            {"symbol": "03690", "name": "美团-W", "reason": "港股生活服务平台", "market": "港股"},
            {"symbol": "01810", "name": "小米集团-W", "reason": "港股智能手机厂商", "market": "港股"}
        ])
    else:
        # A股替代建议 - 基于股票代码前缀提供同类型建议
        if symbol.startswith('6'):  # 上海股票
            suggestions.extend([
                {"symbol": "600000", "name": "浦发银行", "reason": "同为上海交易所银行股", "market": "A股"},
                {"symbol": "600036", "name": "招商银行", "reason": "同为上海交易所银行股", "market": "A股"},
                {"symbol": "600519", "name": "贵州茅台", "reason": "上海交易所蓝筹股", "market": "A股"}
            ])
        elif symbol.startswith('0'):  # 深圳主板
            suggestions.extend([
                {"symbol": "000001", "name": "平安银行", "reason": "同为深圳交易所银行股", "market": "A股"},
                {"symbol": "000002", "name": "万科A", "reason": "深圳交易所蓝筹股", "market": "A股"},
                {"symbol": "000858", "name": "五粮液", "reason": "深圳交易所白酒股", "market": "A股"}
            ])
        else:
            # 通用A股推荐
            suggestions.extend([
                {"symbol": "600000", "name": "浦发银行", "reason": "A股银行龙头", "market": "A股"},
                {"symbol": "600519", "name": "贵州茅台", "reason": "A股白酒龙头", "market": "A股"},
                {"symbol": "000001", "name": "平安银行", "reason": "A股股份制银行", "market": "A股"}
            ])
...
if __name__ == "__main__":
    ...
    for symbol in test_symbols:
        ...
        print(f"股票 {symbol}: 市场={result.get('market', 'N/A')}, 有效={result['is_valid']}, 活跃={result['is_active']}, 名称={result.get('name', 'N/A')}")
