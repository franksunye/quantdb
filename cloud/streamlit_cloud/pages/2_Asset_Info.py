        ...
        if query_button or refresh_button or st.session_state.get('auto_query_asset', False):
            ...
            with st.spinner("资产信息查询中..."):
                try:
                    ...
                    # 显示资产更新控制面板
                    display_asset_update_panel(symbol, asset_service)
...
def display_asset_update_panel(symbol: str, asset_service):
    """显示资产更新控制面板"""
    ...
    with col2:
        ...
        if st.button(
            ...
            type="secondary",
            use_container_width=True,
            help="强制从AKShare获取最新资产信息并更新数据库"
        ):
            # 执行资产更新
            with st.spinner("正在从AKShare获取最新资产信息..."):
                try:
                    if asset_service:
                        # 调用更新服务
                        ...
                        if updated_asset:
                            st.success(f"✅ 成功更新资产 {symbol} 的信息")
                            st.info("页面将自动刷新以显示最新数据")

                            # 清除session state中的缓存数据，强制重新查询
                            if 'current_asset_data' in st.session_state:
                                del st.session_state.current_asset_data
                            if 'current_asset_metadata' in st.session_state:
                                del st.session_state.current_asset_metadata
                            ...
                            st.error(f"❌ 更新失败：未找到资产 {symbol}")
                    else:
                        st.error("❌ 后端服务不可用，无法执行更新操作")

                except Exception as e:
                    st.error(f"❌ 更新过程中出现错误: {str(e)}")
                    with st.expander("🔍 错误详情", expanded=False):
                        st.code(str(e))
    ...
    with st.expander("📋 更新说明", expanded=False):
        st.markdown("""
        **更新内容包括：**
        - 📝 公司名称（如：从"Stock 600000"更新为"浦发银行"）
        - 🏭 行业分类和概念标签
        - 💰 财务指标：PE比率、PB比率、ROE等
        - 📊 市场数据：总市值、总股本、流通股本
        - 📅 上市日期等基本信息
        ...
        **更新频率建议：**
        - 正常情况下，系统会自动缓存数据1天
        - 如发现数据异常，可随时手动更新
        - 财报发布后，建议手动更新获取最新财务指标

        **注意事项：**
        - 更新操作会调用AKShare API，请适度使用
        - 更新完成后页面会自动刷新显示最新数据
        - 如果AKShare服务异常，更新可能失败
        """)
