...
class IndexDataService:
    ...
    def get_realtime_index_data(self, symbol: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary with realtime index data
        """
        logger.info(f"Getting realtime data for index {symbol}")

        try:
            # Check cache first
            cached_data = self._get_cached_realtime_index_data(symbol)
            if cached_data:
                logger.debug(f"Returning cached realtime data for index {symbol}")
                return cached_data

            # Fetch from AKShare
            realtime_data = self.akshare_adapter.get_realtime_index_data(symbol)

            if realtime_data:
                # Save to cache
                self._save_realtime_index_data_to_cache(symbol, realtime_data)
                logger.info(f"Retrieved and cached realtime data for index {symbol}")
                return realtime_data
            else:
                logger.warning(f"No realtime data available for index {symbol}")
                return {}
    ...
    def _get_cached_realtime_index_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        """Get cached realtime index data."""
        try:
            # Check if we have recent cached data (within 5 minutes during trading hours)
            cache_ttl = 300 if self._is_trading_hours() else 3600  # 5 min vs 1 hour
            ...
            cached = (
                ...
                .filter(
                    and_(
                        RealtimeIndexData.symbol == symbol,
                        RealtimeIndexData.capture_time >= cutoff_time,
                    )
                )
                .order_by(desc(RealtimeIndexData.capture_time))
                .first()
            )
            ...
            logger.error(f"Error getting cached realtime index data for {symbol}: {e}")
            return None
    ...
    def _save_realtime_index_data_to_cache(self, symbol: str, data: Dict[str, Any]):
        ...
        try:
            ...
            self.db.add(realtime_data)
            self.db.commit()
            logger.debug(f"Saved realtime index data for {symbol} to cache")

        except Exception as e:
            logger.error(f"Error saving realtime index data for {symbol}: {e}")
            self.db.rollback()
